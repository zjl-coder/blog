...
<Mind :src="$withBase('/images/engineering-mind.md')" />
...
多人合作时，package-lock 频繁冲突，是因为大家使用的 npm 和 node 版本没有统一造成的。
###### engines
...
```
限制项目使用 node 版本大于等于16.13.0，npm 版本 大于等于8.1.0，这个命令**使用 npm 不生效，使用 yarn 可以生效**。

engines 只是建议，默认不开启严格版本校验，只会给出提示，需要手动开启**严格模式**。 
...
<Te s b>解决方法</Te>：husky/pre-commit , 在 git commit 的时候检查 node 和 npm 版本 不符合规定，则终止进程不让提交，或者 husky 其他合适的时间也可以

###### 禁止使用yarn
package.json，只能使用npm 安装包
```json
{
  "scripts": {
